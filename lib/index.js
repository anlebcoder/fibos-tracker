...
let checkVersion = () => {
	let min_version = "v1.5.1.0";

	let now_version = util.buildInfo()["fibos"],
		_now_version = now_version.replace("v", "").split("."),
		_min_version = min_version.replace("v", "").split(".");

	let r = _now_version.every((t, index) => {
		if (t < _min_version[index]) return false;
		else return true;
	})
	if (!r) throw new Error(`fibos version error,require  min version:${min_version}`)
}
...
function Tracker() {
	...
	let putQueues = () => {
		let _eventCaches = eventCaches;

		eventCaches = [];

		Queues.put(_eventCaches);
	}
					...
					try {
						db.trans(() => {
							...
							queues.forEach((queue) => {
								...
								queue.transactions.forEach((trx) => {
	...
	this.emitter = (fb) => {
		...
		fibos.on({
			block: (data) => {
				...
				if (trans.length === 1 && trans[0].action_traces[0].act.name === "onblock" && trans[0].action_traces[0].act.account === "eosio" && Config.isFilterNullBlock) return;
