...
function Tracker() {
	...
	this.emitter = (fb) => {
		...
		fibos.on({
			transaction: autoLock((trx) => {
				let block_num = trx.block_num.toString();
				let producer_block_id = trx.producer_block_id;

				console.warn("transaction b:%s k:%s", block_num, producer_block_id);
				...
				Queues.put("transaction", deepCopy(trx));
			}),
			block: autoLock((bk) => {
				let block_num = bk.block_num.toString();

				console.warn("block b:%s k:%s", block_num, bk.id);
				...
				Queues.put("block", deepCopy(bk));
			}),
			irreversible_block: autoLock((bk) => {
				let block_num = bk.block_num.toString();

				console.warn("irreversible_block b:%s k:%s", block_num, bk.id);
				...
				Queues.put("irreversible_block", deepCopy(bk))
			})
		});
